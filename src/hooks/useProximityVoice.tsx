...
export function useProximityVoice(
  ...
  useEffect(() => {
    ...
    peersRef.current.forEach((peer, uid) => {
      ...
      if (!remote) return;
      const dist = Math.sqrt((myPosition.x - remote.position.x) ** 2 + (myPosition.y - remote.position.y) ** 2);
  ...
  }, [myPosition.x, myPosition.y, remotePlayers, enabled]);
  ...
  useEffect(() => {
    ...
    const handleBroadcast = async (payload: any) => {
      ...
      if (type === 'webrtc-offer') {
        ...
        if (peer) await peer.pc.setRemoteDescription(new RTCSessionDescription(data));
      } else if (type === 'webrtc-ice') {
        const peer = peersRef.current.get(senderId);
        if (peer && data) await peer.pc.addIceCandidate(new RTCIceCandidate(data));
      }
    };

    channel.on('broadcast', { event: 'signal' }, (msg: any) => handleBroadcast(msg.payload));

    return () => {
      // Cleanup is handled by channel removal
    };
  }, [enabled, user, channelRef.current]);
    ...
    pc.onicecandidate = (e) => {
      if (e.candidate && channelRef.current) {
        channelRef.current.send({
          type: 'broadcast',
          event: 'signal',
          payload: { type: 'webrtc-ice', senderId: user.id, targetId: remoteUserId, data: e.candidate },
        });
      }
    };
    ...
    const interval = setInterval(() => {
      remotePlayers.forEach((remote, uid) => {
        ...
        if (dist < VOICE_CONNECT_DISTANCE && !hasPeer) {
          // Only initiator with smaller ID creates offer (avoid duplicate connections)
          if (user.id < uid) {
  ...
  }, [enabled, user, myPosition, remotePlayers, createPeerConnection]);
